    ...
    def _collect_auto_order_parts(self):
        ...
        for cat, key, *rest in _classify_params(self.request.query_params):
            ...
            if cat == "op":  # opérateurs
                # OperatorFilterBackend ignore les opérandes non numériques sans
                # poser l'annotation : trier dessus lèverait un FieldError.
                try:
                    float(rest[1])
...
class PrefixBoostMixin:
    ...
    def _apply_prefix_boost(self, queryset, extra_order_parts=None):
        ...
        for key, value in text_filters:
            ...
            queryset = queryset.annotate(
                **{
                    f"_boost_{key}": Case(
                        When(
                            **{f"{ann}__istartswith": Unaccent(Value(value))},
                            then=Value(0),
                        ),  # SI commence par le terme ALORS 0
                        default=Value(1),  # SINON 1
                        output_field=IntegerField(),  # les 0 remontent plus haut que les 1
                    )
                }
            )
        ...
        order_parts = (
            [f"_boost_{key}" for key, _ in text_filters]
