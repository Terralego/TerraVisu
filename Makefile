...
build_front:
	...
	docker compose -f .docker/frontend/docker-compose.yml run --rm front bash -c "git config --global url.\"https://github.com/\".insteadOf ssh://git@github.com/ && npm ci --legacy-peer-deps && npm run build"
...
format:
	docker compose run --remove-orphans --rm web ruff format project

lint:
	docker compose run --remove-orphans --rm web ruff check --fix project

force_lint:
	docker compose run --remove-orphans --rm web ruff check --fix --unsafe-fixes project
